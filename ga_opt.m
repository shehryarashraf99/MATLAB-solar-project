...
%% hyper parameters
intcon=[]; %% all tilt angles no integer constraints
%intcon=(1:num_trackers); %% all tilt angles have integer constraints
 modifier=swing-0.1;
a=0;
%num_iter=71;
runs=1; %default is 1
...
Self_adjustment_weight=1.5; % default is 0.25
Social_Adjustment_weight=1.05; %default is 0.205
Max_Stall_iterations=2000;%default is NVARS
Max_iterations=2000;
StepTolerance=0.05;
inertia_range=[0.01 1]; % previous is [0.04 0.09]
min_neighbours=0.9;
population=90;
...
% 
% particleoptions= optimoptions('particleswarm', ... 
%                        'FunctionTolerance',Function_Tolerance,...
%                       'UseParallel',true,...
%                        'Hybridfcn',{@patternsearch,patternoptions},... 
%                        'InitialSwarmSpan',initial_velocity,...
%                        'InertiaRange',inertia_range,...
%                        'SelfAdjustmentWeight',Self_adjustment_weight,...
%                        'SocialAdjustmentWeight',Social_Adjustment_weight,...
%                       'MinNeighborsFraction',min_neighbours,...    %default is 0.25                 
%                       'PlotFcn','pswplotbestf',... 
%                        'SwarmSize',num_particles); %default is 100 or 10*NVARS   
...
%      else
%           options.MaxGenerations=Max_iterations;
...
for m=1:runs
   ...
   for i=1:population
     swarm_matrix(i,:)=init{y(i)};
   end
  
%     for i=1:population
%      swarm_matrix(i,:)=init{y(1)}(1,:);
%   end
   
   
   
  options.InitialPopulationMatrix=swarm_matrix;
[potential_tilt_angle{m},~]=ga(objective,NVARS,[],[],[],[],lb,ub,[],intcon,options);
