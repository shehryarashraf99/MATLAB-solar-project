...
%init{i}=repmat(init{i},num_fireflies,1);
...
for m=1:runs
   ...
   for i=1:num_fireflies
     firefly_matrix(i,:)=init{y(i)};
  end
 
[firefly_tilt_angle{m},~]=firefly(firefly_matrix,objective,num_fireflies,alpha,beta0,gamma,theta,Max_iterations,NVARS,lb,ub);
[potential_tilt_angle{m},~]=patternsearch(objective,firefly_tilt_angle{m}(1,:),[],[],[],[],lb,ub,[],patternoptions);
end 
